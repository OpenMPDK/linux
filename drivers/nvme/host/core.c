		...
		struct nvme_command *cmd)
{
	...
	case REQ_OP_ZONE_ZRWA_FLUSH:
		ret = nvme_setup_zone_mgmt_send(ns, req, cmd, NVME_ZONE_ZRWA_FLUSH);
		break;
		...
		struct nvme_ns *ns, struct nvme_id_ns *id)
{
	...
	if (blk_queue_is_zoned(disk->queue))
		ns->nr_zones = id->nsze / nvme_sect_to_lba(ns, ns->zsze);
...
static int nvme_init_subsystem(struct nvme_ctrl *ctrl, struct nvme_id_ctrl *id)
{
	...
	subsys->cmic = id->cmic;
	/* Kanchan: Workaround for HW, multipath creates issues for revalidate-disk */
	subsys->cmic = id->cmic = 0;
