...
static void nvme_zns_clean_zones(struct blk_zone *blk_zones,
...
static int nvme_zone_mgmt_rcv(struct nvme_ns *ns, unsigned int zra,
			      unsigned int zrasf, unsigned int zrasfe,
...
static int nvme_identify_ns_list(struct nvme_ctrl *dev, unsigned int nsid,
				 unsigned int csi, __le32 *ns_list)
{
	struct nvme_command c = { };
...
static int nvme_configure_io_cmd_set(struct nvme_ctrl *ctrl)
{
	...
	ctrl->csc.ncomb = i;
	ctrl->csc.comb = kcalloc(ctrl->csc.ncomb, sizeof(u64), GFP_KERNEL);
	if (!ctrl->csc.comb) {
		ret = -ENOMEM;
		goto out;
	}
...
static void nvme_alloc_ns(struct nvme_ctrl *ctrl, unsigned int nsid, u8 csi)
{
	struct nvme_ns *ns;
	struct gendisk *disk;
	struct nvme_id_ns *id;
...
static int nvme_scan_ns(struct nvme_ctrl *ctrl, unsigned int nn,
			unsigned int csi)
{
	struct nvme_ns *ns;
	struct nvme_ns_ids ids;
	__le32 *ns_list;
	unsigned int i, nsid, prev = 0;
	int ret = 0;
...
{
	u64 iocss = 0;
	unsigned int i, num_lists = DIV_ROUND_UP_ULL((u64)nn, 1024);
	int ret = 0;
...
static int nvme_scan_ns_sequential(struct nvme_ctrl *ctrl, unsigned int nn)
{
	struct nvme_ns_ids ids;
	u64 iocss = 0;
	unsigned int ret, i;
