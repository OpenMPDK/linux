				 ...
				 struct blk_zone *blk_zone)
{
	...
	blk_zone->len = nvme_lbad_to_sec(ns, ns->zone_sz_lb);
...
static int nvme_zns_update(struct nvme_ns *ns)
{
	...
	void *buf;
	sector_t slba = 0;
	int min_sz, buf_sz, max_sz, zone_off = 0, max_hw_bytes;
	int ret;

	min_sz = sizeof(struct nvme_zone_report) + sizeof(struct nvme_zone_desc);
...
static void nvme_config_zoned(struct gendisk *disk, struct nvme_ns *ns)
{
	...
	blk_queue_chunk_sectors(queue,
		1 << get_count_order(ns->zone_sz_lb << (ns->lba_shift - 9)));
...
int nvme_init_identify(struct nvme_ctrl *ctrl)
{
	...
	/* JAVIER: THIS NEEDS TO BE DONE PROPERLY */
	if (NVME_CAP_CSS(ctrl->cap) & NVME_IOCS_MULTIPLE_SUPP) {
		int i;
			...
			if (ctrl->csc.comb[i] & NVME_IOCS_VECTOR_ZND) {
				...
				if (id_zns->zamds)
					ctrl->max_zone_append_sectors =
						1 << (id_zns->zamds + page_shift - 9);
				else
					ctrl->max_zone_append_sectors =
								max_hw_sectors;
				ctrl->zrwacap = id_zns->zrwacap;
				ctrl->mrwz = le32_to_cpu(id_zns->mrwz);
				ctrl->rwanvms = le32_to_cpu(id_zns->rwanvms) >> 9;
				ctrl->zrwacg = (le32_to_cpu(id_zns->zrwacg)) + 1;
