		     ...
		     gfp_t gfp_mask)
{
	...
	if ((op != REQ_OP_ZONE_COMMIT) &&
			(!nr_sectors || end_sector > capacity))
		/* Out of range */
		return -EINVAL;

	/* Check alignment (handle eventual smaller last zone) */
	if ((op != REQ_OP_ZONE_COMMIT) &&
		(zone_sectors == 0 || sector % zone_sectors))
		return -EINVAL;

	if ((op != REQ_OP_ZONE_COMMIT) &&
		(nr_sectors % zone_sectors) && (end_sector != capacity)) {
		return -EINVAL;
	}
				  ...
				  void *data)
{
	...
	if (zone->start == 0) {
		if (zone->len == 0) {
			pr_warn("%s: Invalid device zone size (%llu)\n",
